...
function Navbar() {
  ...
  const nav_routes_ui = nav_routes_array.map((ui, id) => {
    ...
    return (
      <Link style={url === path ? { fontWeight: '700', borderBottom: '2px solid white' } : {}} className='text-[14px] py-[20px] text-white hover:border-b-2 hover:border-b-white ' key={id} to={ui[1]}>{ui[0]}</Link>
    )
  })
  ...
  return (
    <header className="flex flex-col z-50">
      ...
      <div className='flex bg-dark-blue justify-between items-center px-24 py-2 1024:px-4 450:!px-2'>
        ...
        <div className='flex gap-x-10 768:hidden'>
          {nav_routes_ui}
        </div>
        <div>
